...
Features Custom Elements Create and render your own element types on the canvas Important: this is an experimental feature, please proceed with caution. We're kindly asking you to report any bugs and follow the changelog for the most recent updates.
...
If possible, try to use built‑in elements. For example you can draw some complex shapes using svg element (you can generate src at runtime).
...
unstable_registerShapeModel (  
 ...
 Now Polotno store knows that we can define a star model. 2. Create React component for new element  
...
unstable_registerToolbarComponent ( 'star' , StarToolbar);  
 Live demo  
 Canvas Rulers Show and toggle rulers to improve alignment and precision Design Validation Validate canvas designs with custom rules using the Store API
