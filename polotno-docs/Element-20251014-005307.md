...
API Element Elements represent graphical objects in a page or group Element represents a graphical object on the page or inside a group. An element can have one of these types:
...
Combined filters (use element.setFilter ):
...
// ungroup:  
 store. ungroupElements ([group.id]); Video Export Convert Polotno designs to video files using browser-based encoding Page Page API — container for elements inside the store
