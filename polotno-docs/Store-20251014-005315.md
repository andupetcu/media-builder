...
API Store Root data model to control canvas, pages, elements, export, and more Store is a root data model object that you can use to read and control canvas content, check selection, export into different formats and a lot more! It will also provide you with access to Pages and Elements .
...
store. removeFont ( 'MyCustomFont' );  
 store.loadFont(name)  
 Prepare the font to use on the webpage. Text elements inside Workarea will use this function automatically. But it can be useful if you want to show a list of fonts somewhere in the UI. store.loadFont(name) function will add font to the webpage, so the browser can render correct font. Page Page API — container for elements inside the store Animations and Videos Enable animations, preview on canvas, export as GIF, and render MP4 via Cloud API
